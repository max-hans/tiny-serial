...
test('MockSerialPort: open() resolves and sets isOpen', async () => {
...
test('MockSerialPort: multiple writes are concatenated', async () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  await port.open()
  await port.write(Buffer.from('A'))
  await port.write(Buffer.from('B'))
  await port.write(Buffer.from('C'))
...
test('MockSerialPort: clearWrittenData resets history', async () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  await port.open()
  await port.write(Buffer.from('X'))
...
test('MockSerialPort: isOpen starts false before open()', () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  expect(port.isOpen).toBe(false)
})

test('MockSerialPort: open() resolves with no error', async () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  await expect(port.open()).resolves.toBeUndefined()
  expect(port.isOpen).toBe(true)
})

test('MockSerialPort: close() resolves with no error', async () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  await port.open()
...
test('MockSerialPort: mockReply not triggered when pattern not present in write', async () => {
  ...
  await port.write(Buffer.from('SOMETHING_ELSE'))
  await new Promise((resolve) => setTimeout(resolve, 50))
  expect(received).toBe(false)
})
...
test('MockSerialPort: piped through ByteLengthParser emits fixed-size packets', async () => {
  ...
  port.mockReply('go', 'ABCDEF', 0)
  await port.write(Buffer.from('go'))
  await new Promise((resolve) => setTimeout(resolve, 50))
...
test('MockSerialPort: drain() resolves with no error', async () => {
  const port = new MockSerialPort({ path: '/dev/null', baudRate: 9600 })
  await port.open()
